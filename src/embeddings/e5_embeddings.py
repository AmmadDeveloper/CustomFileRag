"""
E5 embeddings implementation using the Hugging Face transformers library.
E5 is a state-of-the-art open-source embedding model from Microsoft that
outperforms many other models including BERT and MiniLM.
"""
import os
import pathlib
...
@EmbeddingFactory.register("e5")
class E5Embeddings(EmbeddingModel):
    """
    Embedding model that uses E5 models from Microsoft via Hugging Face.
    E5 (Embeddings from bidirectional Encoder representations from Transformers) is a
    family of state-of-the-art text embedding models that outperform many other
    open-source models on various benchmarks.
    """

    def __init__(self, model_name: str = "intfloat/e5-large-v2", device: Optional[str] = None, cache_dir: Optional[str] = None):
        """
        Initialize the E5 embeddings model.

        Args:
            model_name: Name of the E5 model to use from Hugging Face
                        Options include:
                        - intfloat/e5-small-v2 (117M parameters)
                        - intfloat/e5-base-v2 (335M parameters)
                        - intfloat/e5-large-v2 (1.3B parameters)
    ...
    def get_embeddings(self, texts: List[str]) -> List[List[float]]:
        """
        Generate embeddings for a list of texts using E5.

        E5 models work best when text is prefixed with "query: " or "passage: "
        depending on whether it's a query or a document. For general use,
        we'll prefix all texts with "passage: ".
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # Prefix texts with "passage: " for better E5 performance
            ...
            with torch.no_grad():
                ...
                for j in range(len(batch_texts)):
                    ...
                    # Normalize the embedding (E5 embeddings should be normalized for best results)
                    normalized_embedding = mean_embedding / torch.norm(mean_embedding)

                    # Convert to list of floats and add to results
                    embeddings.append(normalized_embedding.cpu().tolist())
